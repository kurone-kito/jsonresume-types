...
3. Apply A3's readiness bullets to the target — no configured
   ...
   `jsonresume-types-blocked-by` marker, both resolved the
...
NOT contain a `jsonresume-types-roadmap-id` marker (not itself
a roadmap) or a `jsonresume-types-blocked-by` marker, AND
...
- **Roadmap identity** (`jsonresume-types-roadmap-id`): placed in
...
- **Sequential dependency** (`jsonresume-types-blocked-by`): placed
