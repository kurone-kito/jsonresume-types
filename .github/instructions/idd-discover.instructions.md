...
2. If the target issue carries the configured roadmap label or an
   `jsonresume-types-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `jsonresume-types-blocked-by` markers) and its
...
NOT contain an `jsonresume-types-roadmap-id` marker (not itself
a roadmap) or an `jsonresume-types-blocked-by` marker, AND
...
`jsonresume-types-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`jsonresume-types-roadmap-id` marker. Retro-label a legacy
...
`<!-- jsonresume-types-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `jsonresume-types-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `jsonresume-types-roadmap-id` or
  `jsonresume-types-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `jsonresume-types-roadmap-id` marker when checking
  `jsonresume-types-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- jsonresume-types-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- jsonresume-types-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- jsonresume-types-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- jsonresume-types-effort: S|M|L -->` footer
...
- **Roadmap identity** (`jsonresume-types-roadmap-id`): in the
...
- **Sequential dependency** (`jsonresume-types-blocked-by`): in an
  issue body — this issue **cannot start until** the roadmap with the
  matching `roadmap-id` is closed.

**Do not use `jsonresume-types-blocked-by` to group sub-tasks under
