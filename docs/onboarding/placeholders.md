...
| -------------------------------- | --------------------------------------------------------- | ---------------------------------- |
| `jsonresume-types`                  | Repository short name used in worktree examples           | `my-app`                           |
| `jsonresume-types`      | Hidden issue-body marker prefix                           | `my-app`                           |
...
| `pnpm run lint:fix && pnpm run lint && pnpm run build && pnpm run test` | Post-fix validate command row                             | `npm run lint:fix && npm test`     |
| `corepack enable && pnpm install --frozen-lockfile`       | Dependency install command, or `true` when unnecessary    | `npm install`                      |
...
- no dependency install step →
  `corepack enable && pnpm install --frozen-lockfile = true`
- no relevant auto-fix command →
  `pnpm run lint:fix && pnpm run lint = true`

Keep `corepack enable && pnpm install --frozen-lockfile` safe to rerun
across retries, takeovers, and recreated worktrees.

## Marker prefix notes

`jsonresume-types` appears in two hidden issue-body markers:

- roadmap identity marker:
  `<!-- jsonresume-types-roadmap-id: {unique-id} -->`
- blocked-by marker:
  `<!-- jsonresume-types-blocked-by: {roadmap-id} -->`
